...
const usePostDetails = ({
	id,
}: UsePostDetailsOptions): UsePostDetailsResult => {
	...
	const createPost = async (newPost: Post) => {
		try {
			...
			setDialogMessage(
				'Post created successfully! You will be redirected to the Home page shortly.',
			);
			setTimeout(() => {
				navigate('/');
			}, REDIRECT_TIMEOUT);
	...
	const deletePost = async () => {
		if (id) {
			try {
				...
				setDialogMessage(
					'Post deleted successfully! You will be redirected to the Home page shortly.',
				);
				setTimeout(() => {
					navigate('/');
				}, REDIRECT_TIMEOUT);
